...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.create({
    ...
    password,
    username,
    isEmailVerified: true, // skipping email verification for this project
  });
...
const forgotPasswordRequest=asyncHandler(async (req,res)=>{
    ...
    const user=await User.findOne({email})
    if(!user){
        throw new ApiError(404,"User not present")
    }
    const {unHashedToken,hashedToken,tokenExpiry}=user.generateTemporaryToken();
...
export {registerUser,login,logoutUser,getCurrentUser,verifyEmail,refreshAccessToken,forgotPasswordRequest,resetForgotPassword,resendEmailVerification,changeCurrentPassword};